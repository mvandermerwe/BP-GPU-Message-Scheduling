...
__global__ void rbp_update(device_graph d_graph, device_pgm d_pgm, double* residuals, int* edge_ids, int* edges_effected, int edge_count, double epsilon, int edge_size /* debug */) {
  //----------------------//
...
//   Update residuals   //
//----------------------//
__global__ void rbp_residuals(device_graph d_graph, device_pgm d_pgm, double* residuals, int* edge_ids, int* edges_effected, int edge_count, double epsilon, int edge_size /* debug */) {
...
std::tuple<float, std::vector<double>, int, std::vector<std::pair<int, int>>, std::vector<std::pair<float, int>>> infer(pgm* pgm, double epsilon, int timeout, std::vector<int> runtime_params, bool verbose) {
  ...
  double* d_residuals_out;
  gpuErrchk(cudaMalloc((void**) &d_residuals_out, num_edges * sizeof(double)));
  ...
  while(!converged && time < timeout) {
    ...
    gpuErrchk(cudaMemcpy(top_residual, d_residuals_out, sizeof(double), cudaMemcpyDeviceToHost)); // Only copy back the first value.
